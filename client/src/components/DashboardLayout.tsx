        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          ...
          <div className="flex flex-col gap-3 w-full">
            ...
            {import.meta.env.DEV && (
              ...
              >
                Dev Login (ทดสอบ — ไม่ต้องมี OAuth)
              </Button>
            )}
          </div>
          {import.meta.env.DEV && (
            <p className="text-xs text-muted-foreground text-center">
              ปุ่ม Dev Login ปรากฏเฉพาะ development mode เท่านั้น
            </p>
          )}
        </div>
      </div>
...
function DashboardLayoutContent({
  ...
  const visibleMenuItems = ALL_MENU_ITEMS.filter(item => !item.adminOnly || user?.role === 'admin');
  const activeMenuItem = ALL_MENU_ITEMS.find(item => item.path === location);
          ...
          <SidebarFooter className="p-3">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                {import.meta.env.DEV && (
                  <DropdownMenuItem
                    onClick={() => { window.location.href = "/api/dev-logout"; }}
